...
def draw_boxes(frame, boxes, labels, scores):
    if len(boxes) == 0:
        ...
        cv2.putText(frame, "⚠️ Nothing Detected", (60, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
    else:
...
screenshot_interval = 15  # in seconds
...
while True:
    ...
    for label in labels:
        label_text = class_dict.get(label, "")
        if label_text in unsafe_classes:
            unsafe_detected = True
            break  # even if 1 violation, we handle it

    # ✅ If unsafe & 15s passed since last screenshot
    if unsafe_detected and (current_time - last_screenshot_time >= screenshot_interval):
